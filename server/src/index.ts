...
const userToken =
    process.env["USER_TOKEN"] || "1cde9145-377a-46cb-90be-3e3d765f3409";

const loginRoot = process.env["LOGIN_ROOT"] || "./login-build";
const contentRoot = process.env["CONTENT_ROOT"] || "./content-build";
const cookieExpireDateStr =
    process.env["COOKIE_EXPIRE_DATE"] || "2024-01-01T01:01:01";
...
const setUserIdCookie = (
    ...
    if (req.cookies && !req.cookies.user_id) {
        ...
        res.cookie("user_id", id, {
            expires: new Date(cookieExpireDateStr),
        });
    }
    next();
};
